...
$Subfolders = "/Software/Veeam"
$FilePath = "C:\Temp\NavNet"
$FileName = "VeeamAgentWindows_6.1.0.349.zip"
$Output = @()
...
# Download File
Try {
    Invoke-RestMethod -Uri $Response.'@microsoft.graph.downloadUrl' -OutFile "$($FilePath)\$($FileName)" -Method Get
...
Expand-Archive -Path "$($FilePath)\$($FileName)" -DestinationPath "$($FilePath)"
